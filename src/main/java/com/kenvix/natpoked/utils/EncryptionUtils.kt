...
interface EncryptionUtils {
    ...
    fun decryptToString(cipherBytes: ByteArray, inputOffset: Int, inputLen: Int) =
...
class AES256GCM(keyBytes: ByteArray, keyOffset: Int = 0) : EncryptionUtils {
    private val secretKey = keyBytes.run {
        SecretKeySpec(this, keyOffset, this.size, "AES")
    }
...
class ChaCha20Poly1305(keyBytes: ByteArray, keyOffset: Int): EncryptionUtils {
    private val secretKey = SecretKeySpec(keyBytes, keyOffset, CHACHA20_KEY_SIZE, "CHACHA20POLY1305")
    ...
    override fun decrypt(
        ...
        val cipher = Cipher.getInstance("ChaCha20-Poly1305/NoPadding")  // Get Cipher Instance
        val ivParameterSpec = IvParameterSpec(iv, ivOffset, CHACHA20_IV_LENGTH) // Create GCMParameterSpec
        cipher.init(Cipher.DECRYPT_MODE, secretKey, ivParameterSpec)  // Initialize Cipher for DECRYPT_MODE
